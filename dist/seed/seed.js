...
const generateSalary = (role, designation) => {
    if (role === "Admin")
        return 250000;
    if (role === "HR")
        return 80000;
    if (designation.includes("Director") || designation.includes("Principal"))
        return 220000;
    if (designation.includes("Manager") || designation.includes("Lead"))
        return 150000;
    if (designation.includes("Senior"))
        return 100000;
    if (designation.includes("Specialist") || designation.includes("Analyst"))
        return 70000;
    if (role === "Manager")
        return 120000;
    return 50000;
...
async function seedDatabase() {
    ...
    try {
        ...
        for (let i = 0; i < remainingNeeded; i++) {
            ...
            const role = "Employee";
            const deptManager = employees.find((emp) => emp.department?.toString() === department._id.toString() && emp.role === "Manager");
            const firstName = faker.person.firstName();
            const lastName = faker.person.lastName();
        ...
        console.log(`     - HR: ${employees.filter((e) => e.role === "HR").length}`);
        ...
        for (const dept of departmentDocs) {
            const deptHead = employees.find((emp) => emp.department?.toString() === dept._id.toString() &&
        ...
        for (const employee of employees) {
            ...
            for (let i = 0; i < numLeaves; i++) {
                ...
                const approver = status !== "Pending"
                    ? employees.find((emp) => (emp.role === "Manager" || emp.role === "HR") &&
        ...
        for (const employee of employees) {
            for (let i = 0; i < 6; i++) {
                ...
                await Payroll.create({
                    ...
                    generatedBy: employees.find((e) => e.role === "HR")?._id || employees[0]._id,
                    isActive: true,
                });
                payrollCount++;
            }
