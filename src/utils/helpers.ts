...
/**
 * Calculate PF contribution (12% of basic salary)
 */
 ...
 */
export const calculateProfessionalTax = (salary: number): number => {
...
/**
 * Calculate TDS based on salary (simplified)
 */
export const calculateTDS = (salary: number): number => {
  if (salary <= 500000) return 0;
  if (salary <= 1000000) return salary * 0.05;
  if (salary <= 1500000) return salary * 0.10;
  return salary * 0.15;
};
  ...
  return `
    ...
    <body>
      ...
      <table>
        ...
        ${data.earnings.map((e: any) => `<tr><td>${e.name}</td><td>₹${e.amount.toLocaleString()}</td></tr>`).join('')}
        <tr class="total"><td>Gross Salary</td><td>₹${data.grossSalary.toLocaleString()}</td></tr>
      </table>
      
      <table>
        <tr><th>Deductions</th><th>Amount (₹)</th></tr>
        ${data.deductions.map((d: any) => `<tr><td>${d.name}</td><td>₹${d.amount.toLocaleString()}</td></tr>`).join('')}
