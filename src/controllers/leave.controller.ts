...
export const getAllLeaveRequests = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to fetch leave requests", error: (error as Error).message });
  }
};
...
export const getLeaveRequestById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to fetch leave request", error: (error as Error).message });
  }
};
...
export const createLeaveRequest = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to create leave request", error: (error as Error).message });
  }
};
...
export const updateLeaveRequest = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to update leave request", error: (error as Error).message });
  }
};

// Approve/Reject leave request
export const processLeaveRequest = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const updateData: any = { status, approvedBy: req.user?.id, approvedAt: new Date() };
    ...
    res.status(500).json({ success: false, message: "Failed to process leave request", error: (error as Error).message });
  }
};
...
export const cancelLeaveRequest = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to cancel leave request", error: (error as Error).message });
  }
};
...
export const getLeaveBalance = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    for (const leave of leaves) {
      const type = leave.leaveType as keyof typeof leaveBalances;
      if (leaveBalances[type]) {
        ...
        if (leaveBalances[type].remaining < 0) leaveBalances[type].remaining = 0;
      }
    }
    
    res.status(200).json({ success: true, data: leaveBalances });
  } catch (error) {
    res.status(500).json({ success: false, message: "Failed to fetch leave balance", error: (error as Error).message });
  }
};